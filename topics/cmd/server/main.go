...
func run(logger *zap.SugaredLogger) error {
	props, err := config.ProcessTopicsProperties(logger)
	if err != nil {
		logger.Fatalf("failed to load config: %v", err)
	}

	topicApp, err := setupApplication(props, logger)
	if err != nil {
		return fmt.Errorf("failed to setup application: %v", err)
	}

	return topicApp.Start()
}
func setupApplication(props *config.TopicProperties, logger *zap.SugaredLogger) (*TopicApp, error) {
	...
	return NewTopicApp(logger, props, dbCnx), nil
}
